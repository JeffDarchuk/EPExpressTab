    ...
    public class Initialize
    {
        public void Process(PipelineArgs args)
        {
            ...
            using (new SecurityDisabler())
            {
                ...
                {
                    var tabKey = tab["Placeholder Name"].Substring(3);
                    if (tab["Placeholder Name"].StartsWith("epe"))
                        if (foundTabs.ContainsKey(tabKey))
                            Log.Warn($"Duplicated tab item was found in the tabs folder {Constants.EpTabsFolder}. Ignoring duplicating tab {tabKey}", this);
                        else
                ...
                foreach (string key in EpContext.Tabs.Keys)
                {
                    ...
                    {
                        var tabItemName = ItemUtil.ProposeValidItemName(key);
                        var existingItem = core.GetItem(Constants.EpTabsFolder).Axes.GetDescendant(tabItemName);
                        if (existingItem == null)
                            tab = tabs.Add(tabItemName, new TemplateID(new ID(Constants.EpTabTemplate)));
                    }
